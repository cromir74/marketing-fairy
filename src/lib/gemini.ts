...
- 이모지 최소화`,
        ...
        blog: `네이버 블로그 글을 작성해줘.
- 제목부터 클릭을 유도하는 '후킹' 요소를 넣을 것
- **가장 중요**: 실제 사장님이 직접 운영하며 겪은 '체험형/경험담' 스타일로 작성할 것 (예: "오늘 아침 오픈하면서 이런 일이 있었어요", "이 메뉴를 처음 개발할 때 밤새 고민했던 기억이 나네요")
- 서론의 첫 줄에서 독자의 고민을 건드리거나 호기심을 자극할 것
- SEO 최적화된 제목 포함
- 소제목(##)을 활용한 구조화하되, 문체는 딱딱하지 않은 구어체 사용
- 1000자 이상
- **반복 금지**: 핵심 키워드나 특정 단어가 한 글 내에서 5회 이상 반복되지 않도록 하세요. 다양한 동의어와 풍성한 어휘를 사용하여 독자가 읽기 편한 자연스러운 글을 작성하세요.
- 마지막에 해시태그 5~10개`,
    };
...
3. **블로그는 경험 중심**: 블로그 글은 반드시 '직접 운영하며 겪은 이야기'처럼 서술하세요. 단순 정보 전달이 아니라 사장님의 스토리와 가치관이 담겨야 합니다.
